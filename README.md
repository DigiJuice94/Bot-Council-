...
This package adds an isolated two-stage Council tournament on the V3.6.2 local Council. Claude Risk Reaper has been fully removed from the runtime at the owner's direction: there is no Claude request, veto, latency, scorecard, outcome attribution, or dashboard seat. Deterministic hard-risk rules, zero-liquidity protection, confirmed sellability/locked-capital detection, Executor feasibility, Guardian exits and Audit Watch remain active.

- Qualifier: Team 1 preserves the exact baseline BUY threshold. Teams 2–9 use controlled aggressive, balanced and defensive role/threshold/sizing variations across a meaningful 52–58 point selection range. Team 10 is **Team File Cabinet**, which uses the existing Runner Genome, trajectory, winner/dumper and missed-runner evidence as advisory input only.
...
- Independent decisions: Team 1 is the exact local-Council baseline control. Teams 2–10 synthesize the same locked specialist reads with their own controlled weighting/threshold variation; a shared soft SKIP no longer forces every experimental team to pass. Hard safety and Executor BLOCK remain global.
- Visible pass reasons: each ranking row reports the latest exact reason that team declined an opportunity.
- Selection separation: experimental teams can act on local-Council WATCH opportunities when their specialty-adjusted score crosses their own threshold. The earlier 56–58 range was too narrow and made almost every team mirror the baseline. Confirmed hard-risk and Executor blocks remain global and cannot be overridden.
- Active-state migration: an already-running qualifier keeps its wallets, positions, results and clock, but reloads the current team names, biases, sizing and thresholds by team ID. Old near-identical settings cannot remain trapped in Redis after deployment.
- Fair restart: this correction uses the V2 tournament ledger namespace, so the earlier zero-trade test is excluded and a new 24-hour qualifier begins on deployment.
- Faster bounded discovery: three chains are scanned concurrently by default, allowing provider and Council latency to overlap and cycling all seven chains in three bounded batches instead of seven serial passes. Global wallet/research maintenance still runs once per batch, and simultaneous tournament decisions wait for the shared ledger instead of dropping one. `WAR_ROOM_SCAN_WORKERS` can be set from `1` to `3`; the worker pool remains hard-capped at `3` to avoid recreating the earlier Node memory problem.
