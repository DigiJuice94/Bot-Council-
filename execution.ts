...
export async function executePaper(request: ExecutionRequest, snapshot: MarketSnapshot): Promise<PaperFill> {
  ...
  // V3.6 deliberately makes the generic paper executor BUY-only. All PAPER sells
  // must flow through position-manager's verified reverse-route quote so a modeled
  // mark can never be booked as realized proceeds.
  if (request.side === "SELL") {
    throw new Error("Paper SELL rejected: V3.6 requires a verified reverse sell-route quote before proceeds can be realized.");
  }
  // Final execution backstop: no caller can create a PAPER position from a market
  // snapshot reporting zero liquidity, even if upstream fields are inconsistent.
  ...
  if (observedSlippageBps > request.maxSlippageBps) {
    throw new Error(`Paper BUY rejected: estimated live-route/liquidity impact ${observedSlippageBps} bps exceeds ${request.maxSlippageBps} bps limit.`);
  }

  const simulatedSlippageBps = Math.min(request.maxSlippageBps, observedSlippageBps);
  const priceImpact = 1 + simulatedSlippageBps / 10_000;
  const feeRate = snapshot.chainFamily === "solana" ? 0.0015 : 0.0025;
  const grossFilledUsd = request.notionalUsd;
